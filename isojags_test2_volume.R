...
#Here we have defined alpha as volume weighted for each source.


alpha = matrix(1, nrow=Ntotal, ncol=Ktotal)
alpha = rep(1, Ktotal)
## Could do alpha as dist to source^-1 * volume
#alpha = distM^-1 * isource$volumes
...
modelString = "
  model {
    ...
    for (i in 1:N) {
      # Generate mu
      for (j in 1:J) {
        mu[i,j] = inprod(p[i,1:K],s_mean[1:K,j])
      }
...
diagMCMC(codaObject = codaSamples, parName ="p[64,4]")



#MCMC <- as.mcmc(codaSamples)
#output <-ggs(MCMC)
#ggmcmc(output, file = "12345.pdf"")
